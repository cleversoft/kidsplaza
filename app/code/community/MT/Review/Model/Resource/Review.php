 ...
 *
 */
class MT_Review_Model_Resource_Review extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
     * Review table
     *
    ...
    {
        $this->_init('mtreview/review', 'review_id');
        $this->_reviewTable         = $this->getTable('mtreview/review');
        $this->_reviewDetailTable   = $this->getTable('mtreview/review_detail');
        $this->_reviewStatusTable   = $this->getTable('mtreview/review_status');
        $this->_reviewEntityTable   = $this->getTable('mtreview/review_entity');
        $this->_reviewStoreTable    = $this->getTable('mtreview/review_store');
        $this->_aggregateTable      = $this->getTable('mtreview/review_aggregate');
        $this->_reviewHelpfulnessTable     = $this->getTable('mtreview/review_helpfulness');
    }
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        if($object->getParentReviewId()){
            $detail['parent_review_id'] = $object->getParentReviewId();
            if($object->getParentReviewId()){
                $level = Mage::getModel('mtreview/review')->load($object->getParentReviewId())->getLevel();
                $level += 1;
                $detail['level'] = $level;
            }else{
                $detail['level'] = 0;
