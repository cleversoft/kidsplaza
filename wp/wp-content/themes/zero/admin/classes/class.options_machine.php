...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		global $bbit_output, $bbit_details, $bbit_data;
		if (empty($options))
			return;
		if (empty($bbit_data))
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($bbit_data) || !empty($data)){
			
				if (array_key_exists('id', $value) && !isset($bbit_data[$value['id']])) {
					$bbit_data[$value['id']] = $value['std'];
					if ($value['type'] == "checkbox" && $value['std'] == 0) {
						$bbit_data[$value['id']] = 0;
					} else {
						$update_data = true;
					}
				}
				if (array_key_exists('id', $value) && !isset($bbit_details[$value['id']])) {
					$bbit_details[$value['id']] = $bbit_data[$value['id']];
				}
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				if (array_key_exists("fold",$value)) {
					if (isset($bbit_data[$value['fold']]) && $bbit_data[$value['fold']]) {
						$fold="f_".$value['fold']." ";
					} else {
			...
			//if (!isset($bbit_data[$value['id']]) && $value['type'] != "heading")
			//	continue;
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($bbit_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
						$output .= '<option id="' . $select_ID . '" value="'.$theValue.'" ' . selected($bbit_data[$value['id']], $theValue, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($bbit_data[$value['id']]);			
						$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
				break;
				
				//radiobox option
				case "radio":
					$checked = (isset($bbit_data[$value['id']])) ? checked($bbit_data[$value['id']], $option, false) : '';
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($bbit_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($bbit_data[$value['id']])) {
						$bbit_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($bbit_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					(isset($bbit_data[$value['id']]))? $multi_stored = $bbit_data[$value['id']] : $multi_stored="";
				...
				case "color":
					...
					$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $bbit_data[$value['id']] . '"' . $default_color .' />';
				...
				case 'typography':
					...
					$typography_stored = isset($bbit_data[$value['id']]) ? $bbit_data[$value['id']] : $value['std'];
					...
					if(isset($typography_stored['style'])) {
						...
						$styles = array('normal'=>'Thường',
										'italic'=>'In nghiêng',
										'bold'=>'In đậm',
										'bold italic'=>'In nghiêng và đậm');
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $bbit_data[$value['id']];
				...
				case 'images':
					...
					$select_value = (isset($bbit_data[$value['id']])) ? $bbit_data[$value['id']] : '';
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($bbit_data[$value['id']]) && !empty($bbit_data[$value['id']]) ? $bbit_data[$value['id']] : '';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($bbit_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
				...
				case 'select_google_font':
					...
					foreach ($value['options'] as $select_key => $option) {
						$output .= '<option value="'.$select_key.'" ' . selected((isset($bbit_data[$value['id']]))? $bbit_data[$value['id']] : "", $option, false) . ' />'.$option.'</option>';
					} 
					$output .= '</select></div>';
					...
					$hide = " hide";
					if ($bbit_data[$value['id']] != "none" && $bbit_data[$value['id']] != "")
						$hide = "";
				...
				case 'sliderui':
					...
					$s_val  = stripslashes($bbit_data[$value['id']]);
					...
					$output .= '<div id="'.$value['id'].'-slider" class="bbit_sliderui" style="margin-left: 7px;" '. $s_data .'></div>';
				...
				case 'switch':
					...
					$output .= '<p class="switch-options">';
						...
						$output .= '<input type="hidden" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
						$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($bbit_data[$value['id']], 1, false) .' />';
				...
				case "media":
					...
					$u_val = '';
					if($bbit_data[$value['id']]){
						$u_val = stripslashes($bbit_data[$value['id']]);
					}
