...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		global $apl_output, $apl_details, $apl_data;
		if (empty($options))
			return;
		if (empty($apl_data))
			$apl_data = of_get_options();
		$data = $apl_data;
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($apl_data) || !empty($data)){
			
				if (array_key_exists('id', $value) && !isset($apl_data[$value['id']])) {
					$apl_data[$value['id']] = $value['std'];
				...
				if (array_key_exists('id', $value) && !isset($apl_details[$value['id']])) {
					$apl_details[$value['id']] = $apl_data[$value['id']];
				}
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				if (array_key_exists("fold",$value)) {
					if (isset($apl_data[$value['fold']]) && $apl_data[$value['fold']]) {
						$fold="f_".$value['fold']." ";
					} else {
			...
			//if (!isset($apl_data[$value['id']]) && $value['type'] != "heading")
			//	continue;
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($apl_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
						$output .= '<option id="' . $select_ID . '" value="'.$theValue.'" ' . selected($apl_data[$value['id']], $theValue, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($apl_data[$value['id']]);			
						$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
				break;
				
				//radiobox option
				case "radio":
					$checked = (isset($apl_data[$value['id']])) ? checked($apl_data[$value['id']], $option, false) : '';
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($apl_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($apl_data[$value['id']])) {
						$apl_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($apl_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					(isset($apl_data[$value['id']]))? $multi_stored = $apl_data[$value['id']] : $multi_stored="";
				...
				case "color":
					...
					$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $apl_data[$value['id']] . '"' . $default_color .' />';
				...
				case 'typography':
					...
					$typography_stored = isset($apl_data[$value['id']]) ? $apl_data[$value['id']] : $value['std'];
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $apl_data[$value['id']];
				...
				case 'images':
					...
					$select_value = (isset($apl_data[$value['id']])) ? $apl_data[$value['id']] : '';
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($apl_data[$value['id']]) && !empty($apl_data[$value['id']]) ? $apl_data[$value['id']] : '';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($apl_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
				...
				case 'select_google_font':
					...
					foreach ($value['options'] as $select_key => $option) {
						$output .= '<option value="'.$select_key.'" ' . selected((isset($apl_data[$value['id']]))? $apl_data[$value['id']] : "", $option, false) . ' />'.$option.'</option>';
					} 
					$output .= '</select></div>';
					...
					$hide = " hide";
					if ($apl_data[$value['id']] != "none" && $apl_data[$value['id']] != "")
						$hide = "";
				...
				case 'sliderui':
					...
					$s_val  = stripslashes($apl_data[$value['id']]);
					...
					$output .= '<div id="'.$value['id'].'-slider" class="apl_sliderui" style="margin-left: 7px;" '. $s_data .'></div>';
				...
				case 'switch':
					...
					$output .= '<p class="switch-options">';
						...
						$output .= '<input type="hidden" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
						$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($apl_data[$value['id']], 1, false) .' />';
				...
				case "media":
					...
					$u_val = '';
					if($apl_data[$value['id']]){
						$u_val = stripslashes($apl_data[$value['id']]);
					}
