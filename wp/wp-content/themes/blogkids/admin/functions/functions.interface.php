...
function optionsframework_admin_init() 
{
	...
	global $of_options, $options_machine, $apl_data, $apl_details;
	...
	if (empty($apl_data['apl_init'])) { // Let's set the values if the theme's already been active
		of_save_options($options_machine->Defaults);
		of_save_options(date('r'), 'apl_init');
...
function optionsframework_add_admin() {
    ...
    $of_page = add_theme_page( THEMENAME, 'Cài đặt APL', 'edit_theme_options', 'apl_framework', 'optionsframework_options_page');
...
function of_load_only() 
{
	...
	wp_enqueue_script('apl', ADMIN_DIR .'assets/js/apl.js', array( 'jquery' ));
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$apl_data = of_get_options(BACKUPS);
	...
	elseif($save_type == 'import_options'){
		...
		$apl_data = unserialize(base64_decode($_POST['data'])); //100% safe - ignore theme check nag
		of_save_options($apl_data);
	...
	elseif ($save_type == 'save')
	{
		...
		wp_parse_str(stripslashes($_POST['data']), $apl_data);
		unset($apl_data['security']);
		unset($apl_data['of_save']);
		of_save_options($apl_data);
